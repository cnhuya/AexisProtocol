module deployer::testEntities12{
    ...
    use deployer::testCore31::{Self as Core, Entity, Type, Stat, StatString, Location, Material, MaterialString };
...
public fun getMultipleEntitiesByIDs(ids: vector<u8>): vector<String> acquires Entity_Database {
    ...
    let entities = &entity_db.database;  // <-- borrow from database vector, which holds Entities
    let len = vector::length(entities);
    let lenIds = vector::length(&ids);
    let vect = vector::empty<String>();
    let i = len;
    while (i > 0) {
        i = i - 1;
        let entityID = vector::borrow(&ids, i);
        let entity_ref = vector::borrow(entities, i);
        if (Core::get_entity_ID(entity_ref) == *entityID) {
            vector::push_back(&mut vect, Core::get_entity_name(entity_ref));
        };
    };
    move vect
}
