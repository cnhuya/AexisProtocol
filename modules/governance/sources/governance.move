module deployer::governance{
    ...
    use std::table;
    use std::debug::print;
    use 0x1928893148d317947c302185417e2c1d32640c6ef8521b48e1ae6308ab1a41c3::points;
    use std::string::utf8;
    ...
    // 1 = NUMBER
    // 2 = BOOL
    // 3 = ADDRESS (HIERARCH ID)
    struct PROPOSAL has copy, key, store, drop {id: u32, hash: vector<u8>, proposer: address, modul: u32, code: u16, name: vector<u8>, desc: vector<u8>, start: u64, end: u64, stats: PROPOSAL_STATS, status: PROPOSAL_STATUS, from: u256, to: u256, type: u8}
    ...
    struct HIERARCH has  key, store, drop {id: u64, address: address, name: vector<u8>, code: u8}
    ...
    struct HISTORICAL_PROPOSALS has copy, drop, store, key {database: vector<PROPOSAL>}
     ...
     entry fun init_module(address: &signer) acquires HIERARCHY, HIERARCH_DATABASE {
        ...
        if (!exists<PROPOSAL>(deploy_addr)) {
            ...
            move_to(address, PROPOSAL { id: 0, hash: b"0",proposer: @0x0, modul: 0, code: 0, name: vector::empty(), desc: vector::empty(), start: 0, end: 0, stats: proposal_stats, status: proposal_status, from: 0, to: 0, type:0});
        };
        ...
        if (!exists<HIERARCHY>(deploy_addr)) {
            ...
            let database = borrow_global<HIERARCH_DATABASE>(DEPLOYER);

            let hierarch =  HIERARCH {
                id: vector::length(&database.database),
                address: deploy_addr,
                name: b"OWNER",
                code: 10,
            };
    ...
    public entry fun addHierarch(address: &signer, _hierarch: address, _name: vector<u8>, _code: u8) acquires HIERARCHY, HIERARCH_DATABASE {
        ...
        let hierarch = HIERARCH {
            id: vector::length(&hierarch_db.database),
            address: _hierarch,
            name: _name,
            code: _code,
        };
    ...
    public entry fun vote(address: &signer, modul: u32, code: u16, yes: bool) acquires MODULE_TABLE, HISTORICAL_PROPOSALS, PROPOSAL_COUNTER, VOTERS_TABLE {
        ...
        assert!(points::viewBalance(signer::address_of(address)) > 0, ERROR_NOT_ENOUGH_BALANCE);
        ...
        if (!table::contains(&voters_table.voters, modul)) {
            ...
            let _vote = VOTE {
                value:  points::viewBalance(signer::address_of(address)),
                yes: yes,
            };
        ...
        assert!(points::viewBalance(signer::address_of(address)) > 0, ERROR_NOT_ENOUGH_BALANCE);
        if(yes == true){
            proposal.stats.yes = proposal.stats.yes + points::viewBalance(signer::address_of(address));   
        } else {
            proposal.stats.no = proposal.stats.no + points::viewBalance(signer::address_of(address)); 
        };
    ...
    entry fun innitializeProposal(address: &signer, _name: vector<u8>, _module: u32, _code: u16, _desc: vector<u8>, _period: u8, _from: u256, _to: u256, type:u8) acquires MODULE_TABLE, HISTORICAL_PROPOSALS, PROPOSAL_COUNTER {
        assert!(signer::address_of(address) == OWNER || signer::address_of(address) == DEPLOYER, ERROR_NOT_OWNER);
    ...
    public entry fun createProposal(address: &signer, _name: vector<u8>,  _moduleId: u32,  _moduleCode: u16, _desc: vector<u8>, _period: u8, _from: u256, _to: u256, type:u8) acquires PROPOSAL_COUNTER, HISTORICAL_PROPOSALS, MODULE_TABLE
    {
        innitializeProposal(address, _name, _moduleId, _moduleCode, _desc, _period, _from, _to, type);
    }

    public entry fun changeProposal(address: &signer, _moduleId: u32, _code: u16, _name: vector<u8>, _desc: vector<u8>, type: u8) acquires MODULE_TABLE {
        assert!(signer::address_of(address) == OWNER || signer::address_of(address) == DEPLOYER, ERROR_NOT_OWNER);
   ...
   public entry fun deleteProposal(address: &signer, _id:u32)  acquires PROPOSAL_COUNTER, HISTORICAL_PROPOSALS{
    ...
    #[view]
    public fun viewHierarch(address: address): (u64,address,vector<u8>,u8) acquires HIERARCHY {
        ...
        (hierarch.id,address,hierarch.name, hierarch.code)
    }
    ...
    public fun viewHierarchy(): vector<HIERARCH> acquires HIERARCH_DATABASE, HIERARCHY {
        ...
        while(length > i){
            ...
            let (id, _address,hierarch, code) = viewHierarch(hierarch_addr);
    ...
    #[view]
    public fun viewProposalByModule_tuple(_module: u32, _code: u16): (u32,address,u16,bool,bool,u256,u256,u8) acquires MODULE_TABLE
    {
        let data = viewProposalByModule(_module, _code);
        (data.id, data.proposer, data.code, data.status.pending, data.status.passed, data.from, data.to, data.type)
    }
    ...
    #[test(account = @0x1, owner = @0x392727cb3021ab76bd867dd7740579bc9e42215d98197408b667897eb8e13a1f, acc1 = @0xfff1ffff2fff3ff44ff5)]
    public entry fun test(account: signer, owner: signer, acc1: signer) acquires HIERARCH_DATABASE, HIERARCHY, HISTORICAL_PROPOSALS, PROPOSAL_COUNTER, MODULE_TABLE, VOTERS_TABLE{
        ...
        init_module(&owner);
        print(&viewHierarchy());
        createProposal(&owner, b"Change Owner Address", 0, 1, b"Change owner address", 7, 100, 200,1);
        createProposal(&owner, b"Testing for Module 0000", 0, 5, b"Testing proposal", 7, 200, 100,2);
        createProposal(&owner, b"Testing for Module 1", 1, 1, b"Testing proposal", 7, 1000, 500,3);
